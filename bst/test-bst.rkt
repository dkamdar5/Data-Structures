  ...
  (test-suite
    ...
    (test-case "Multi-Element BST --- Functional Test"
     (let ((a (make-bst <)))
       ...
       (check-equal? (bst-node-right (bst-root d)) (bst-node 3 'three '() (bst-node 5 'five '() '())) "4 deleted, and 2 children")
       (define e (add b 6 'six))
       (check-equal? (bst-size e) 6 "Increment size")
       (define f (delete e 5))
       (check-equal? (bst-size f) 5 "Decrement size")
       (check-equal? (bst-node-right (bst-node-right (bst-root f))) (bst-node 6 'six '() '()) "5 deleted, and 1 child")
